...
export function Header({ children }: HeaderProps) {
  return (
    <header className="flex items-center justify-between px-4 sm:px-6 h-14 border-b border-border/60 bg-background/80 backdrop-elevated">
      <Link
        to="/dashboard"
        className="flex items-center gap-2 sm:gap-3 h-full transition-opacity hover:opacity-80 min-w-0"
      >
        <Logo />
          ...
          <SignedOut>
            <SignInButton mode="modal">
              <Button
                variant="outline"
                size="sm"
                className="h-7 rounded-sm font-medium px-2 sm:px-4 text-xs sm:text-sm"
