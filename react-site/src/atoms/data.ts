...
// Style/theme atom - basic version (detailed one moved to style.ts)
export const basicStyleAtom = atom({
  theme: 'default',
  colors: {
    primary: '#000000',
    secondary: '#666666',
...
// Derived atom that handles CSS injection
export const cvDataAtom = atom(
  (get) => get(baseCvDataAtom),
  (_get, set, newData: any) => {
    const currentData = _get(baseCvDataAtom);
    set(baseCvDataAtom, newData);
    // Inject CSS when it changes
    if (newData.css !== undefined && newData.css !== currentData.css) {
      injectCss('resume-editor', newData.css);
    }
  }
)